...
// Update 更新字段定义（内置字段的 FieldKey 不可改）
...
// UpsertFromSync 以「同步来源」的字段定义写入本地（按 field_key 幂等更新）。
//
// 用于从 FlyIAM 同步字段定义：已存在则更新展示属性，不存在则新增；
// 内置字段也允许被同步更新（但保留 Builtin 标记与 field_key）。
func (l *Logic) UpsertFromSync(ctx context.Context, d *model.UserFieldDef) error {
	if err := normalize(d); err != nil {
		return err
	}
	query := `
		...
		ON CONFLICT (field_key) DO UPDATE SET
			...
			-- 本地内置标记保持：内置字段任何情况下都不可删除
			builtin = user_field_defs.builtin OR EXCLUDED.builtin,
			updated_at = NOW()
	`
	if _, err := l.db.ExecCtx(ctx, query,
		d.FieldKey, d.Label, d.FieldType, d.Options, d.ShowInList, d.ShowInForm, d.Editable, d.Builtin, d.SortOrder,
	); err != nil {
		return fmt.Errorf("同步字段定义失败（%s）: %w", d.FieldKey, err)
	}
	return nil
}
