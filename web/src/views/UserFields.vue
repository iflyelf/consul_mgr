    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <div class="title">
            <span>用户字段</span>
            <el-tag type="info" size="small">与 FlyIAM 对齐，数据源字段变化无需改代码</el-tag>
          </div>
          <div class="header-actions">
            <el-button type="primary" :icon="Plus" @click="openCreate">新增字段</el-button>
            <el-button :icon="Refresh" :loading="syncing" @click="handleSync">从 FlyIAM 同步</el-button>
      ...
      <el-alert
        ...
        :closable="false"
        show-icon
        title="字段键对应用户属性（Casdoor Properties）的键名；内置字段不可删除，可改为不显示。"
...
const handleSync = async () => {
  try {
    await ElMessageBox.confirm('从 FlyIAM 同步字段定义（按字段键幂等更新）？', '提示', { type: 'info' })
  } catch (e) {
    return
  }
  syncing.value = true
...
<style scoped>
.user-fields-container { width: 100%; }
...
.form-tip { font-size: 12px; color: var(--text-secondary); line-height: 1.6; }
</style>
